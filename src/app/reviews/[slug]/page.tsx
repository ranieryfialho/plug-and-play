...
import { Star, CheckCircle2, XCircle, ShoppingCart, Calendar, User, Tag, Facebook } from "lucide-react";
...
// --- QUERY BLINDADA (Sem resumoVeredito) ---
const PAGE_QUERY = `
...
export default async function ReviewPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const currentUrl = `https://plug-play.com/reviews/${slug}`; 
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-border pb-6 gap-4">
           <div className="flex items-center gap-3">
              <div className="h-12 w-12 rounded-full bg-secondary/50 border border-border flex items-center justify-center relative overflow-hidden">
                  {authorAvatarUrl ? (
                    <Image src={authorAvatarUrl} alt={authorName} fill className="object-cover" sizes="48px" />
        ...
        <div className="lg:col-span-8">
          {post.featuredImage?.node?.sourceUrl && (
            <div className="relative w-full h-[300px] md:h-[450px] rounded-xl overflow-hidden mb-8 border border-border shadow-lg">
               <Image src={post.featuredImage.node.sourceUrl} alt={post.title} fill className="object-cover" priority sizes="(max-width: 768px) 100vw, (max-width: 1200px) 66vw" />
            </div>
          )}
            ...
            <div className="mb-10 bg-card border border-border rounded-xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-md hover:border-primary/40 transition-colors group">
               <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-2xl font-bold text-white">{post.camposDoReview.precoAtual || "Ver Oferta"}</p>
                  </div>
               </div>
               <Button asChild className="w-full sm:w-auto bg-primary hover:bg-secondary text-white font-bold shadow-[0_0_15px_rgba(124,58,237,0.4)] cursor-pointer">
                  <a href={post.camposDoReview.linkDeAfiliadoMlolx} target="_blank" rel="noopener noreferrer">
                    <ShoppingCart className="mr-2 h-4 w-4" /> Ir para Loja
                  </a>
               </Button>
            </div>
          )}
          ...
          <article 
            className="prose prose-invert prose-lg max-w-none prose-headings:text-white prose-p:text-gray-300 prose-strong:text-white prose-a:text-primary hover:prose-a:text-accent prose-ul:text-gray-300 prose-li:marker:text-primary"
        ...
        <div className="lg:col-span-4 space-y-8">
          <div className="sticky top-24">
            <Card className="bg-card border-border p-6 shadow-2xl shadow-primary/5 mb-8">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-xl font-bold text-white flex items-center gap-2">
                  <Star className="w-5 h-5 text-primary fill-primary" /> Veredito
                </h3>
              ...
              <p className="text-sm text-muted-foreground mb-6 italic border-l-2 border-primary pl-3">"{summary}"</p>

              <div className="space-y-4 mb-6">
                 {prosList.slice(0, 3).map((pro: string, i: number) => (
                    <div key={i} className="flex items-start gap-2 text-sm text-gray-300">
                       <CheckCircle2 className="w-4 h-4 text-green-500 shrink-0 mt-0.5" /><span>{pro}</span>
                    </div>
                 ))}
                 {consList.slice(0, 3).map((con: string, i: number) => (
                    <div key={i} className="flex items-start gap-2 text-sm text-gray-300">
                       <XCircle className="w-4 h-4 text-red-500 shrink-0 mt-0.5" /><span>{con}</span>
                    </div>
                 ))}
              </div>

              {post.camposDoReview?.linkDeAfiliadoMlolx && (
                <Button asChild className="w-full bg-green-600 hover:bg-green-700 text-white font-bold cursor-pointer">
                  <a href={post.camposDoReview?.linkDeAfiliadoMlolx} target="_blank" rel="noopener noreferrer">Comprar Agora</a>
                </Button>
              )}
            </Card>
            ...
            <div className="bg-card/50 border border-border rounded-xl p-5">
               <h4 className="font-bold text-white mb-4 flex items-center gap-2">⚡ Últimas do Blog</h4>
                    ...
                    <Link key={recent.id} href={`/reviews/${recent.slug}`} className="flex gap-3 group">
                       <div className="relative w-20 h-16 rounded-md overflow-hidden bg-secondary shrink-0">
                          {recent.featuredImage?.node?.sourceUrl && (
                            <Image src={recent.featuredImage.node.sourceUrl} alt={recent.title} fill className="object-cover group-hover:scale-110 transition-transform" sizes="80px" />
                          )}
                       </div>
                       <div>
                          <h5 className="text-sm font-medium text-gray-200 leading-snug group-hover:text-primary transition-colors line-clamp-2">{recent.title}</h5>
                          <span className="text-[10px] text-muted-foreground">{formatDistanceToNow(new Date(recent.date), { addSuffix: true, locale: ptBR })}</span>
                       </div>
                    </Link>
                  ))}
               </div>
